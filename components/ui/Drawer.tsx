...
export const drawerVariants = cva(
  "fixed z-20 flex w-full flex-col border-black/10 bg-white p-6 outline-hidden! duration-200 motion-reduce:transition-none dark:border-neutral-800 dark:bg-[#161617]",
  {
    variants: {
      variant: {
...
export const DrawerOverlay = ({ className, ...props }: React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Overlay>) => (
  <DrawerPrimitive.Overlay className={cn("fixed inset-0 bg-black/50 dark:bg-[#161617]/70", className)} {...props} />
);
...
export const DrawerTitle = ({ ref, className, ...props }: React.ComponentPropsWithRef<typeof DrawerPrimitive.Title>) => (
  <DrawerPrimitive.Title
    ref={ref}
    className={cn("flex gap-2 text-xl font-medium text-neutral-900 duration-200 motion-reduce:transition-none dark:text-white", className)}
    {...props}
  />
);

export const DrawerDescription = ({ ref, className, ...props }: React.ComponentPropsWithRef<typeof DrawerPrimitive.Description>) => (
  <DrawerPrimitive.Description ref={ref} className={cn("text-base text-neutral-600 dark:text-neutral-400", className)} {...props} />
);
