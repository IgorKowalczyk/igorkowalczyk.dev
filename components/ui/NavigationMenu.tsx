...
const NavigationMenuViewport = ({ className, ...props }: React.ComponentPropsWithRef<typeof NavigationMenuPrimitive.Viewport>) => (
 <div className="">
  <NavigationMenuPrimitive.Viewport
   className={cn(
    "text-text viewport data-[state=closed]:animate-scale-out data-[state=open]:animate-scale-in absolute mt-2 h-[var(--radix-navigation-menu-viewport-height)] w-full overflow-hidden rounded-xl border border-black/10 bg-white/90 shadow-lg backdrop-blur-xl transition-[width,_height] duration-300 md:w-[var(--radix-navigation-menu-viewport-width)] dark:border-neutral-800 dark:bg-[#161617]/70",
    className
   )}
   {...props}
  />
...
const NavigationMenuList = ({ className, ...props }: React.ComponentPropsWithRef<typeof NavigationMenuPrimitive.List>) => (
 <NavigationMenuPrimitive.List className={cn("group flex flex-1 list-none items-center justify-center space-x-2", className)} {...props} />
);
...
const NavigationMenuContent = ({ className, ...props }: React.ComponentPropsWithRef<typeof NavigationMenuPrimitive.Content>) => (
 <NavigationMenuPrimitive.Content
  className={cn(
   "data-[motion=from-end]:animate-enter-from-right data-[motion=from-start]:animate-enter-from-left data-[motion=to-end]:animate-exit-to-right data-[motion=to-start]:animate-exit-to-left top-0 left-0 w-full md:absolute md:w-auto",
   className
  )}
  {...props}
 />
...
const NavigationMenuIndicator = ({ className, ...props }: React.ComponentPropsWithRef<typeof NavigationMenuPrimitive.Indicator>) => (
 <NavigationMenuPrimitive.Indicator
  className={cn(
   "origin-top-center origin-top-center data-[state=hidden]:animate-scale-out data-[state=visible]:animate-scale-in top-full z-10 flex h-2 items-end justify-center overflow-hidden transition-[width,transform_250ms_ease]",
   className
  )}
  {...props}
 >
